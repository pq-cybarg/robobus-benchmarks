...
### The full-stack portability harness (local, no paid CI)

Hosted CI minutes are a paywall, so the portability proof is written to run on **one
workstation**. `bench/portability_harness.sh` starts a container for each ISA (Docker + QEMU
binfmt) and runs the whole stack inside it: the **wire codec**, **schema codegen**, the **crypto
correctness verify**, the **Rust / Node / Java** language bindings, and the **transport bridges**,
asserting each seals frames the Python reference opens and vice-versa on that architecture. It
emits `bench/results/portability-matrix.json`, which the site renders as the **Portability** grid
on the [Speed matrix](../speed.html#portability). Timing under emulation is not cycle-accurate and
is deliberately not recorded; this axis proves the stack **computes correctly** across endianness,
word size and alignment (the big-endian s390x cell is the point). Reproduce anywhere Docker + QEMU
are registered:

```bash
colima start && docker run --privileged --rm tonistiigi/binfmt --install all
bench/portability_harness.sh linux/arm64 linux/s390x linux/ppc64le linux/riscv64 linux/arm/v7 linux/386
```
